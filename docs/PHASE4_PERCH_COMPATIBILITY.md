...
- 本機分析入口：`scripts/local_analysis.py`，驗證 import、query、Decimal calculation、provenance 與 deterministic hash。
...
原 Perch 相容性 Gate **未通過／blocked**；Perch Desktop／CLI／Web 需要付費 runtime，Stan 已決定不在本機訂閱或部署 Perch。本次重規劃採 A+B：Phase 4 改以本機分析介面作為 MVP Gate，Perch 僅保留為未來外部匯出目標，不把本機測試宣稱為 Perch 相容性證據。設計決策記錄於 `docs/PHASE4_REPLAN.md`。
